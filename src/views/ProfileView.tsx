...
import { beers, formatBeerTitle, getUniqueParticipantPosts, resolvePokedexEntryBeer, type Beer } from '../beers';
...
export const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  Object.entries(myPokedex || {}).forEach(([key, entry]) => {
    if (entry && entry.rating && entry.rating > 0) {
      ...
      const { beerType } = resolvePokedexEntryBeer(key, entry, catalog);
            ...
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '30px' }}>
              ...
              <div style={{ textAlign: 'left' }}>
                ...
                <div style={{ fontSize: '12px', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '4px', marginTop: '3px' }}>
                  <span className="material-symbols-outlined" style={{ fontSize: '14px', color: 'var(--primary-dark)' }}>group</span>
                  <span>{taggedFriendsCount} Brindisi in compagnia</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div style={{ padding: '0 20px', animation: 'fadeIn 0.2s ease-out', marginBottom: '30px' }}>
          ...
          >
            ...
            {(() => {
              ...
              Object.entries(myPokedex || {}).forEach(([key, entry]) => {
                if (!entry) return;
                const { brand: b, variant: v, beerType } = resolvePokedexEntryBeer(key, entry, catalog);
                if (!b || !v) return;
